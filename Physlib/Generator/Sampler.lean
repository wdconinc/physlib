...
/-- The acceptance weight, `F₂(x) · Y(y) / Q²`, up to constants common to every trial. -/
def weight (x q2v y : Float) : Float := f2Toy x q2v * yFactor y / q2v

/-- A numerical overestimate of `weight` on the region: `F₂^max / Q²_min`, with `F₂^max`
found by scanning `n` points logarithmically in `x` and inflated by `safety`.

Not analytic — see the module docstring.  `safety` defaults to 1.3. -/
